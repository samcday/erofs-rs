...
impl<R: ReadAt> EroFS<R> {
    ...
    async fn read_compressed_inode_range(
        ...
        while written < out.len() && offset < inode.data_size() {
            ...
            if extent.encoded {
                let data = self.get_or_decode_extent_data(inode.id(), &meta, &extent).await?;
                let end = in_extent + n;
    ...
    async fn get_or_decode_extent_data(
        ...
        let out = match extent.algorithm {
            Z_EROFS_COMPRESSION_LZ4 => {
                ...
                let decoded =
                    lz4_flex::block::decompress_into(&compressed, &mut out).map_err(|err| {
                        Error::CorruptedData(format!("lz4 decompress failed: {err}"))
                    })?;
    ...
    async fn load_lcluster(
        ...
        match inode.layout()? {
            Layout::CompressedFull => {
                self.load_full_lcluster(meta, total_lclusters, lcn).await
            }
    ...
    async fn load_compact_lcluster(
        ...
        if kind == Z_EROFS_LCLUSTER_TYPE_NONHEAD {
            rec.clusterofs = 1usize << meta.lclusterbits;
            if lookahead {
                rec.delta1 = compacted_lookahead_distance(lobits, encodebits, vcnt, &pack, i)?
                    as u16;
            }
